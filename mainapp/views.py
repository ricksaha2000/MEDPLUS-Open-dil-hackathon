...
def external(request):
    ...
    out= run([sys.executable,'C:\\Users\\Jayit\\Downloads\\MEDHEART-master\\medeasy\\clean_code.py'],shell=True,stdout=PIPE)
    print(out)
    return render(request,'mainapp/HomePage.html',{'data1':out.stdout})

def chatbot(request):
    inp= request.POST.get('param')
    out= run([sys.executable,'C:\\Users\\Jayit\\Downloads\\MEDHEART-master\\medeasy\\Medical-Chatbot-master\\QuestionDiagonosisTkinter.py'],shell=True,stdout=PIPE)
    print(out)
    return render(request,'mainapp/HomePage.html',{'data2':out.stdout})
